...
func (s *SearchService) SearchByMedia(ctx context.Context, fileHeader *multipart.FileHeader, limit int) ([]models.SearchResult, error) {
	...
	log.Printf("Performing similarity search (Type: %s, Limit: %d)", mediaType, limit)
	switch mediaType {
	case models.ImageType:
		searchResults, err = s.repo.FindSimilarImages(ctx, queryVector, limit)
	case models.AudioType:
		searchResults, err = s.repo.FindSimilarAudio(ctx, queryVector, limit)
	}
